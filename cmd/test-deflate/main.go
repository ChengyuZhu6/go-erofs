...
func main() {
	...
	flag.StringVar(&imgPath, "img", "", "Path to DEFLATE compressed erofs image")
	...
	fmt.Printf("Testing DEFLATE compressed EROFS image: %s\n", imgPath)
	...
	err = fs.WalkDir(img, "/", func(path string, entry fs.DirEntry, err error) error {
		...
		if ok {
			...
			// 如果是压缩文件，尝试读取内容
			if st.InodeLayout == 1 || st.InodeLayout == 3 {
	...
	fmt.Println("\n✓ DEFLATE compressed EROFS image test completed successfully")
}
